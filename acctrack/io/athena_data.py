...
def true_edges(hits):
    hit_list = hits.groupby(
        ...
        "layer_disk",
        "eta_module",
        "phi_module",], sort=False)['index'].agg(
            lambda x: list(x)).groupby(level=0).agg(lambda x: list(x))
...
class AthenaDFReader:
    ...
    def read(self, evtid=None):
        ...
        truth_fname = prefix + f"-truth.{postfix}"
        particle_fname = prefix + f"-particles.{postfix}"
        
        try:
            truth = pd.read_csv(truth_fname)
        except:
            truth = pd.read_pickle(truth_fname)
        ...
        try:
            ...
            particles = pd.read_pickle(particle_fname)
        ...
        good_hits = good_hits.assign(R=np.sqrt((good_hits.x - good_hits.vx)**2 \
            + (good_hits.y - good_hits.vy)**2 + (good_hits.z - good_hits.vz)**2))
        good_hits = good_hits.sort_values('R')
        edges = true_edges(good_hits)

        data = MeasurementData(hits=None, measurements=None, 
