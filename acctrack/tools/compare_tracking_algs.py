...
class TrackAlgComparator(HyperparametersMixin):
    ...
    def compare_track_contents(self) -> Tuple[Any, Any, Any]:
        ...
        if not self.redo_comparison and self.common_info is not None:
            return self.common_track_indices, self.unmatched_tracks, self.disjoint_track_indices
        ...
        print(f"Total # of {label} tracks: {tot_tracks}. After filtering, "
        ...
        print(f"{label} is a subset: {num_issubset}, {tot_filtered_tracks}, "
              f"{num_issubset/tot_filtered_tracks:.4f}")
        print(f"{other_label} is a subset: {num_other_issubset}, {tot_filtered_tracks}, "
        ...
        self.common_track_indices, self.unmatched_tracks, self.disjoint_track_indices = \
    ...
    def plot_disjoint_tracks(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        plt.hist([len(x[1]) for x in disjoint_tracks], **cluster_config,
                 label=f"{label} {num_disjoints}")
        plt.hist([len(x[1]) for x in disjoint_other_tracks], **cluster_config,
        ...
        good_disjoints = tracks_matched_to_truth[tracks_matched_to_truth.trkid.isin(disjoint_track_indices)]
        ...
        print(f"Number of good disjoint {other_label} tracks: {num_other_good_disjoints} / {num_other_disjoints}")
        ...
        other_good_disjoints_tot_hits = other_good_disjoints.reco_pixel_hits + other_good_disjoints.reco_sct_hits

        plt.title("Good Disjoint Tracks")
        plt.hist(good_disjoints_tot_hits, **cluster_config,
                 label=f"{label} {num_good_disjoints}/{num_disjoints}")
        plt.hist(other_good_disjoints_tot_hits, **cluster_config,
        ...
        matched_particles = particles[particles.particle_id.isin(good_disjoints.particle_id.values)]
        other_matched_particles = particles[particles.particle_id.isin(other_good_disjoints.particle_id.values)]
        ...
        plt.scatter(merged.pt,
                    merged.reco_pixel_hits + merged.reco_sct_hits, label=f"{label}", **config)
        plt.scatter(merged_other.pt,
                    merged_other.reco_pixel_hits + merged_other.reco_sct_hits, label=f"{other_label}", **config)
        plt.xlim(0, 5000)
        plt.ylim(0, 30)
        plt.xlabel("particle pT [MeV]")
