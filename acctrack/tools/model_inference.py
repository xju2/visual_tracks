...
def batched_inference(
    model, senders, receivers, batch_size: int = 1024, debug: bool = False
):
    results = []
    ...
    def inference(
        ...
        if self.stage_name == "graph_construction":
            ...
            edge_index = build_edges(
                embedding, r_max=radius, k_max=knn, backend=knn_backend
            )
            (
                truth_labels,
                true_edges,
...
class ExaTrkxInference:
    def __init__(
        ...
        self.data_reader = TrackGraphDataReader(
            data_path, name=self.name + "DataReader"
        )
        self.edge_perf = EdgePerformance(self.data_reader)

        e_config = config["embedding"]
        self.embedding_model = torch.jit.load(
            e_config["model_path"], map_location=self.device
        )
        self.embedding_model.eval()
        self.embedding_model.hparams = e_config

        f_config = config["filtering"]
        self.filtering_model = torch.jit.load(
            f_config["model_path"], map_location=self.device
        )
        ...
        if g_config is not None:
            self.gnn_model = torch.jit.load(
                g_config["model_path"], map_location=self.device
            )
            self.gnn_model.eval()
    ...
    def __call__(
        ...
        **kwargs
        ...
        edge_index = build_edges(
            embedding, r_max=r_max, k_max=k_max, backend=knn_backend
        )
        ...
        if self.gnn_model is not None:
            ...
            gnn_perf = self.edge_perf.eval_edge_scores(
                gnn_edge_scores, truth_labels, outname="perf_gnn_evt{}".format(evtid)
            )
