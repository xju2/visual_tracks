...
class TrackAnalyzer:
    ...
    def study_cluster_features(self, evtid: int = 0):
        ...
        if self.reader.clusters is None:
            print(
                f"No cluster information found in {self.reader.name} for event {evtid}."
            )
        ...
        ax.hist(
            pixel_clusters["glob_eta"], label=r"global $\eta$", **config, linestyle="--"
        )
        ax.hist(
            pixel_clusters["glob_phi"], label=r"global $\phi$", **config, linestyle="--"
        )
        ...
        ax.hist(
            strip_clusters["glob_eta"], label=r"global $\eta$", **config, linestyle="--"
        )
        ax.hist(
            strip_clusters["glob_phi"], label=r"global $\phi$", **config, linestyle="--"
        )
        ax.set_xlabel("Cluster Shapes")
        plt.legend()
        plt.show()
    ...
    def apply_eta_dep_cuts(
        ...
        num_failed_all = track[
            num_hits_cuts | pt_cuts | outlier_cuts | chi2_cuts
        ].shape[0]
        print("Total number of tracks: ", track.shape[0])
        print(
            "Number of tracks failed number of hits cut: ",
            num_failed_hits,
            f"({num_failed_hits/track.shape[0]*100:.2f}%)",
        )
        print(
            "Number of tracks failed pT cut: ",
            num_failed_pt,
            f"({num_failed_pt/track.shape[0]*100:.2f}%)",
        )
        print(
            "Number of tracks failed outlier cut: ",
            num_failed_outliers,
            f"({num_failed_outliers/track.shape[0]*100:.2f}%)",
        )
        print(
            "Number of tracks failed chi2 cut: ",
            num_failed_chi2,
            f"({num_failed_chi2/track.shape[0]*100:.2f}%)",
        )
        print(
            "Number of tracks failed all cuts: ",
            num_failed_all,
            f"({num_failed_all/track.shape[0]*100:.2f}%)",
        )
